...
public class MotherLongitudinalFragment extends Fragment {
    ...
    private void confirmDeleteVisit(MotherLongitudinalFollowUpModel visit) {
    ...
    private void deleteVisit(MotherLongitudinalFollowUpModel visit) {
        final View progress = binding != null ? binding.progressLoading : null;
        if (progress != null) progress.setVisibility(View.VISIBLE);
        Threading.io(() -> {
            MotherLongitudinalFollowUpDao.deleteByRowId(visit.getDb_row_id());
            Threading.main(() -> {
                if (!isAdded()) return;
                try {
