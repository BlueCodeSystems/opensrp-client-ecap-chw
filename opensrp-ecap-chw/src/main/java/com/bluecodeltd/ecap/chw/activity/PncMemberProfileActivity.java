...
public class PncMemberProfileActivity extends CorePncMemberProfileActivity implements PncMemberProfileContract.View {
        ...
        switch (requestCode) {
            ...
            case JsonFormUtils.REQUEST_CODE_GET_JSON:
                try {
                    ...
                    } else if (form.getString(JsonFormUtils.ENCOUNTER_TYPE).equals(EventType.PNC_REFERRAL)) {
            ...
            case Constants.REQUEST_CODE_HOME_VISIT:
                this.setupViews();
                ChwScheduleTaskExecutor.getInstance().execute(memberObject.getBaseEntityId(), EventType.PNC_HOME_VISIT, new Date());
    ...
    private void refreshOnHomeVisitResult() {
        Observable<Visit> observable = Observable.create(e -> {
            Visit lastVisit = getVisit(EventType.PNC_HOME_VISIT);
            e.onNext(lastVisit);
            e.onComplete();
        });
    ...
    protected void startFpRegister() {
        FpRegisterActivity.startFpRegistrationActivity(this, memberObject.getBaseEntityId(), memberObject.getDob(), JSON_FORM.getFpRegistrationForm("Female"), FamilyPlanningConstants.ActivityPayload.REGISTRATION_PAYLOAD_TYPE);
    }

    @Override
    protected void startFpChangeMethod() {
        FpRegisterActivity.startFpRegistrationActivity(this, memberObject.getBaseEntityId(), memberObject.getDob(), JSON_FORM.getFpChangeMethodForm("female"), FamilyPlanningConstants.ActivityPayload.CHANGE_METHOD_PAYLOAD_TYPE);
    }
