...
public class MotherPostnatalFragment extends Fragment {
    ...
    private void confirmDeleteVisit(MotherPostnatalCareModel visit) {
    ...
    private void deleteVisit(MotherPostnatalCareModel visit) {
        final View progress = binding != null ? binding.progressLoading : null;
        if (progress != null) progress.setVisibility(View.VISIBLE);
        Threading.io(() -> {
            MotherPostnatalCareDao.deleteByRowId(visit.getDb_row_id());
            Threading.main(() -> {
                if (!isAdded()) return;
                try {
