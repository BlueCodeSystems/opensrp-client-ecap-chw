...
import ecap.smartregister.chw.model.FamilyProfileDueModel;
import ecap.smartregister.chw.presenter.FamilyProfileDuePresenter;
import ecap.smartregister.chw.provider.ChwDueRegisterProvider;
...
public class FamilyProfileDueFragment extends BaseFamilyProfileDueFragment {
    ...
    private void getNextAction(View view, Bundle fragmentArguments) {
        ...
        if (view.getTag() instanceof CommonPersonObjectClient) {
            ...
            if (scheduleName.equalsIgnoreCase(CoreConstants.SCHEDULE_TYPES.WASH_CHECK)) {
                startForm(ecap.smartregister.chw.util.Constants.JSON_FORM.getWashCheck(), ecap.smartregister.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_WASH);
            } else if (scheduleName.equalsIgnoreCase(CoreConstants.SCHEDULE_TYPES.FAMILY_KIT)) {
                startForm(ecap.smartregister.chw.util.Constants.JSON_FORM.getFamilyKit(), ecap.smartregister.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_FAMILY_KIT);
            } else if (scheduleName.equalsIgnoreCase(CoreConstants.SCHEDULE_TYPES.ROUTINE_HOUSEHOLD_VISIT)) {
                startForm(ecap.smartregister.chw.util.Constants.JSON_FORM.getRoutineHouseholdVisit(), ecap.smartregister.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_HOUSEHOLD);
            } else {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case ecap.smartregister.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_WASH:
            case ecap.smartregister.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_FAMILY_KIT:
            case ecap.smartregister.chw.util.JsonFormUtils.REQUEST_CODE_GET_JSON_HOUSEHOLD:
                if (resultCode == Activity.RESULT_OK) {
                    try {
                        ...
                        switch (encounterType) {
                            case ecap.smartregister.chw.util.Constants.EventType.WASH_CHECK:
                            case ecap.smartregister.chw.util.Constants.EventType.ROUTINE_HOUSEHOLD_VISIT:
                                ((FamilyProfileDuePresenter) presenter).saveData(jsonString);
                                break;
                            case ecap.smartregister.chw.util.Constants.EventType.FAMILY_KIT:
